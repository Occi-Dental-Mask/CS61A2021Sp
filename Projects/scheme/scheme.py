...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        args = rest.map(lambda x:scheme_eval(x,env))#The map method of Pair returns a new Scheme list constructed by applying a one-argument function to every item in a Scheme list
...
def eval_all(expressions, env):
    ...
    #((print 1) 2) ->Pair(Pair('print',Pair(2,nil)),Pair(2,nil))
    # END PROBLEM 7
...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        if symbol in self.bindings:
            ...
            return self.parent.lookup(symbol)#当前frame找不到，就去它的parent frame找（递归地找）
        # END PROBLEM 2
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 11
        "*** YOUR CODE HERE ***"
        new_frame = self.env.make_child_frame(self.formals, args)#call时，创造一个新的frame,这个frame的parent frame是定义该函数时创建的frame
        return new_frame
        # END PROBLEM 11
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # expr.rest.first -> Pair('+', Pair(2, Pair(8, nil)))   #from github.com
        rest_eval = scheme_eval(expressions.rest.first,env)
        env.define(target, rest_eval)
        return target
        # END PROBLEM 5
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        #expr = read_line("((f x) (+ x 2))")
        #((f x ) (+ x 2)) -> Pair(Pair('f', Pair('x', nil)) ,Pair('+', Pair('x', Pair(2, nil))))
        #target-> Pair('f', Pair('x', nil))
        #target.first->'f' 函数名字
        #target.rest-> Pair('x', nil) 一众函数参数
        #body-> Pair('+', Pair('x', Pair(2, nil)))
        body = expressions.rest
        name = target.first
        formals = target.rest
        LambdaProcedure_instance = do_lambda_form(Pair(formals,body),env)
        env.define(name,LambdaProcedure_instance)
        return name
        # END PROBLEM 9
    else:
...
def do_quote_form(expressions, env):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    return expressions.first#Pair(A, nil), where:A is the quoted expression
    # END PROBLEM 6
...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    "*** YOUR CODE HERE ***"
    # Pair('#f',Pair(Pair('print',Pair(1,nil))),nil)
    # Pair(1, Pair('#f',nil))
    exp = expressions
    if exp == nil:
        return True
    while exp.rest != nil:
        if is_false_primitive(scheme_eval(exp.first,env)):
            return False
        else:
            exp = exp.rest
    return scheme_eval(exp.first,env)
...
def do_or_form(expressions, env):
    ...
    """
    # BEGIN PROBLEM 12
    # exp -> Pair('#f', Pair(2, Pair(3, Pair('#t',Pair('#f", nil)))))
...
def do_cond_form(expressions, env):
    ...
    3
    """
    # Pair(Pair('#f',Pair(Pair('print',Pair(2,nil),nil),Pair('#t',Pair(3,nil)))
...
def do_let_form(expressions, env):
    ...
    5
    """
    #Pair( Pair(x,Pair(2,nil))   , Pair(Pair(y,Pair(3,nil))))
    #Pair(Pair( Pair(x,Pair(2,nil)),Pair(Pair(y,Pair(3,nil))))   , Pair(Pair('+',Pair(x,Pair(y,nil)))))
    #BINGDINGS->  Pair(   Pair(Pair(x,Pair(2,nil)),nil) ,  Pair(Pair(y,Pair(3,nil)),nil)   )
    #p.first ->Pair(Pair(x,Pair(2,nil)),nil)
    #p.first.first ->Pair(x,Pair(2,nil))
    validate_form(expressions, 2)
...
def make_let_frame(bindings, env):
    ...
    p = bindings
    while p != nil:
        validate_form(p.first,2,2)
        name = p.first.first
        names = Pair(name,names)
        value = scheme_eval(p.first.rest.first,env)
