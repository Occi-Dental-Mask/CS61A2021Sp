...
def scheme_read(src):
    ...
    if val == 'nil':
        ...
        # BEGIN PROBLEM 1
        "*** YOUR CODE HERE ***"
        return read_tail(src)#src已经在第122行被更新了
        # END PROBLEM 1
    elif val == "'":
        # BEGIN PROBLEM 6
        "*** YOUR CODE HERE ***"
        ...
        # END PROBLEM 6
    elif val not in DELIMITERS:#it must be boolean or number
        return val
    else:
...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            # BEGIN PROBLEM 1
            "*** YOUR CODE HERE ***"
            first = scheme_read(src)#scheme_read the next complete expression in the buffer.
            rest = read_tail(src)#Call read_tail to read the rest of the combination until the matching closing parenthesis.
            return Pair(first,rest)#Return the results as a Pair instance, where the first element is the next complete expression from (1) and the second element is the rest of the combination from (2).
            # END PROBLEM 1
    except EOFError:
        raise SyntaxError('unexpected end of file')
