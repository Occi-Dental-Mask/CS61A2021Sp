    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.is_container() and place.ant.can_contain(self):
                place.ant.contain_ant(self)#注意：self指的是要add的蚂蚁
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        self.place_temp = self.place
        while True:
            if self.place_temp.bees == []:
                if self.place_temp.is_hive():
                    return None
                else:
                    self.place_temp = self.place_temp.entrance
                    self.distance += 1
            else:
                if self.place_temp.is_hive():
                    return None
                elif self.distance<=self.max_range and self.distance>=self.min_range:
                    return bee_selector(self.place_temp.bees)
                else:
                     self.place_temp = self.place_temp.entrance
                     self.distance += 1
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health > amount:
            Ant.reduce_health(self, amount)#注意，在这里直接调用父类方法
            ...
            for bee in self.place.bees[:]:
                bee.reduce_health(amount)
            for bee in self.place.bees[:]:#注意，在对列表进行循环时不能对列表进行修改
...
class HungryAnt(Ant):
    ...
    def action(self,gamestate):
        if self.chewing == 0:#它不在嚼蜜蜂
            if self.place.bees !=[]:#位置上有蜜蜂
                bee_a = bee_selector(self.place.bees)
                #bee_a.health = 0#某一只可怜蜜蜂health变为0
                #bee_a.remove_from(bee_a,self.place)#被移出place
                bee_a.reduce_health(bee_a.health)#注意：self不用写到参数里去,self就是指调用这个方法的对象
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8
        "*** YOUR CODE HERE ***"
        if not other.is_container() and self.contained_ant == None:#如果加入的蚂蚁不是容器蚂蚁并且容器蚂蚁不包含另一只蚂蚁
            return True
        # END Problem 8
    ...
    def action(self, gamestate):
        ...
        if self.contained_ant != None:
            self.contained_ant.action(gamestate)#不是定义，而是调用，不需要加self
        # END Problem 8
