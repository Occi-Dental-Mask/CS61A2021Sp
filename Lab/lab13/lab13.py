...
def roman_numerals(text):
    ...
    []
    """
    return re.findall(r'\bM{0,3}(CM|CD|D?C{0,3})(XC|XL|L?X{0,3})(IX|IV|V?I{0,3})[\b\.]', text)
...
def calculator_ops(calc_str):
    ...
    []
    """
    return re.findall(r'\([*-+] [0-9] [0-9]\)',calc_str)
...
def cs_classes(post):
    ...
    False
    """
    return bool(re.search(r'CS\d\d[A|B|C]', post))
