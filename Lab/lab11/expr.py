...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        operator_value = self.operator.eval(env)#找出操作符表达式指代的值
        lst = []
        #lst = list(self.operands.map(eval))
        for operand in self.operands:
            lst.append(operand.eval(env))#计算每个操作数
        return operator_value.apply(lst)
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        z_dic = dict(zip(self.parameters,arguments))#复制当前环境
        for key in z_dic.keys():
            d_copy[key] = z_dic[key]
