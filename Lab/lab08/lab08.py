...
def subseqs(s):
    ...
    if len(s) == 1:
        return [seq] + [s]
    if len(s) == 2:
        return [seq] + [s[1:]] + [s[:-1]] + [s]
    if len(s) == 3:
        return [seq] + [s[1:]] + [s[:-1]] + [s[2:]] + [s[:-2]] + [s]
    else:
        return subseqs(s[1:]) + subseqs(s[:-1]) + [seq]
...
def my_append(a,b):
    if a is Link.empty:
        return b
    return Link(a.first,my_append(a.rest,b))

def uncompress(s):
    if s is Link.empty:
        return Link.empty
    return my_append(replicate(s.first.first,s.first.rest.first),uncompress(s.rest))
#elif s.rest is Link.empty:
#return replicate(s.first.first,s.first.rest.first)Just help to understand
...
def enumerate_me(s):
    ...
    while s is not Link.empty:
        temp = Link(count,Link(s.first))
        final = Link(temp,final)
        count += 1
        s = s.rest
    final_reverse = Link.empty
    p = final
    while p is not Link.empty:
        final_reverse = Link(p.first,final_reverse)
        p = p.rest
...
def enumer(s):
    def en(s,n):
        if s is Link.empty:
            return s
        return Link(Link(n,Link(s.first)),en(s.rest,n+1))
    return en(s,0)
